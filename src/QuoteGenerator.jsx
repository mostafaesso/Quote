...
export default function QuoteGenerator() {
  ...
  // Groups raw HubSpot text into numbered "items" — each item starts at a
  // line like "1-" or "2." and absorbs any following non-numbered lines
  // (which are just manual line-wraps from how it was typed in HubSpot).
  // Joining them with spaces lets the browser reflow the whole item to fill
  // the page width naturally, instead of breaking wherever the original
  // text happened to wrap.
  const groupNumberedItems = (str) => {
    const lines = String(str).split('\n').map(l => l.trim()).filter(l => l.length > 0);
    const items = [];
    lines.forEach(line => {
      if (/^\d+\s*[.\-]/.test(line)) {
        items.push({ header: line, body: [] });
      } else if (items.length > 0) {
        items[items.length - 1].body.push(line);
      } else {
        items.push({ header: null, body: [line] });
      }
    });
    return items;
  };
